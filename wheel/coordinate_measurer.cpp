...
void CoordinateMeasurer::update(const std::array<int32_t,4> &encoder_count){
 ...
 * @param attachment_radius 計測輪の取付半径
 */
CoordinateMeasurerSquare::CoordinateMeasurerSquare(
	uint16_t encoder_PPR, uint16_t radius_of_measure_Wheel, uint16_t attachment_radius
):
	CoordinateMeasurer(encoder_PPR, radius_of_measure_Wheel, attachment_radius)
{
 ...
 * @return ロボット座標の微小移動距離
 */
Coordinate<float> CoordinateMeasurerSquare::convert_to_robot(std::array<float,4> distance){
	Coordinate<float> micro_robot_distance;
	static std::array<float,4> prev_distance = distance;
 ...
 * @return
 */
float CoordinateMeasurerSquare::calc_angle(std::array<float,4> distance){
	return (distance[0] + distance[1] + distance[2] + distance[3])/(4.0*parameter.attachment_radius);
}
